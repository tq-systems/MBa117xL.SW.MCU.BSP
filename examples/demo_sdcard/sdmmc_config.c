 ...
 * Definitions
 ******************************************************************************/
#define MBA117xL_SDHC1_RESET_GPIO GPIO10
#define MBA117xL_SDHC1_RESET_PIN 2U
...
AT_NONCACHEABLE_SECTION_ALIGN(
  static uint32_t
    s_sdmmcHostDmaBuffer[BOARD_SDMMC_HOST_DMA_DESCRIPTOR_BUFFER_SIZE],
...
SDK_ALIGN(
  static uint8_t
    s_sdmmcCacheLineAlignBuffer[BOARD_SDMMC_DATA_BUFFER_ALIGN_SIZE * 2U],
...
void BOARD_USDHC_Errata( void )
{
  /* ERR050396
   * Errata description:
   * AXI to AHB conversion for CM7 AHBS port (port to access CM7 to TCM) is by a
   * NIC301 block, instead of XHB400 block. NIC301 doesn’t support sparse write
   * conversion. Any AXI to AHB conversion need XHB400, not by NIC. This will
   * result in data corruption in case of AXI sparse write reaches the NIC301
   * ahead of AHBS. Errata workaround: For uSDHC, don’t set the bit#1 of
   * IOMUXC_GPR28 (AXI transaction is cacheable), if write data to TCM aligned
   * in 4 bytes; No such write access limitation for OCRAM or external RAM
   */
...
{
  return GPIO_PinRead( BOARD_SDMMC_SD_CD_GPIO_BASE,
...
{
  if ( GPIO_PortGetInterruptFlags( BOARD_SDMMC_SD_CD_GPIO_BASE ) &
       ( 1U << BOARD_SDMMC_SD_CD_GPIO_PIN ) ) {
...
void BOARD_SDCardDAT3PullFunction( uint32_t status )
{
  if ( status == kSD_DAT3PullDown ) {
    IOMUXC_SetPinConfig( IOMUXC_GPIO_SD_B1_05_USDHC1_DATA3,
                         0xCU ); /* no pull in IOMUX configuration, 100K pull
                                    down externally is required. */
...
void BOARD_SDCardDetectInit( sd_cd_t cd, void *userData )
{
  ...
  if ( BOARD_SDMMC_SD_CD_TYPE == kSD_DetectCardByGpioCD ) {
    ...
    GPIO_PinInit( BOARD_SDMMC_SD_CD_GPIO_BASE, BOARD_SDMMC_SD_CD_GPIO_PIN,
                  &sw_config );
    GPIO_PortEnableInterrupts( BOARD_SDMMC_SD_CD_GPIO_BASE,
                               1U << BOARD_SDMMC_SD_CD_GPIO_PIN );
    ...
    if ( GPIO_PinRead( BOARD_SDMMC_SD_CD_GPIO_BASE,
                       BOARD_SDMMC_SD_CD_GPIO_PIN ) ==
         BOARD_SDMMC_SD_CD_INSERT_LEVEL ) {
      if ( cd != NULL ) {
        cd( true, userData );
...
void BOARD_SDCardPowerResetInit( void )
{
  ...
  GPIO_PinInit( BOARD_SDMMC_SD_POWER_RESET_GPIO_BASE,
                BOARD_SDMMC_SD_POWER_RESET_GPIO_PIN, &sw_config );
}

void BOARD_SDCardPowerControl( bool enable )
{
  if ( enable ) {
    GPIO_PinWrite( MBA117xL_SDHC1_RESET_GPIO, MBA117xL_SDHC1_RESET_PIN, 1 );
  }
  else {
    GPIO_PinWrite( MBA117xL_SDHC1_RESET_GPIO, MBA117xL_SDHC1_RESET_PIN, 0 );
  }
}
#endif

#ifdef SD_ENABLED
void BOARD_SD_Config( void *card, sd_cd_t cd, uint32_t hostIRQPriority,
                      void *userData )
{
  assert( card );
  ...
  ( (sd_card_t *)card )->usrParam.maxFreq =
    BOARD_SDMMC_SD_HOST_SUPPORT_SDR104_FREQ;
...
#ifdef SDIO_ENABLED
void BOARD_SDIO_Config( void *card, sd_cd_t cd, uint32_t hostIRQPriority,
                        sdio_int_t cardInt )
{
  assert( card );
  ...
  ( (sdio_card_t *)card )->host->hostController.base =
    BOARD_SDMMC_SDIO_HOST_BASEADDR;
  ( (sdio_card_t *)card )->host->hostController.sourceClock_Hz =
    BOARD_USDHC1ClockConfiguration();
  ...
  ( (sdio_card_t *)card )->usrParam.maxFreq =
    BOARD_SDMMC_SD_HOST_SUPPORT_SDR104_FREQ;
  if ( cardInt != NULL ) {
...
void BOARD_MMC_Config( void *card, uint32_t hostIRQPriority )
{
  ...
  ( (mmc_card_t *)card )->host->hostController.base =
    BOARD_SDMMC_MMC_HOST_BASEADDR;
  ( (mmc_card_t *)card )->host->hostController.sourceClock_Hz =
    BOARD_USDHC1ClockConfiguration();
  ( (mmc_card_t *)card )->host->tuningType    = BOARD_SDMMC_MMC_TUNING_TYPE;
  ( (mmc_card_t *)card )->usrParam.ioStrength = NULL;
  ( (mmc_card_t *)card )->usrParam.maxFreq =
    BOARD_SDMMC_MMC_HOST_SUPPORT_HS200_FREQ;

  ( (mmc_card_t *)card )->usrParam.capability |=
    BOARD_SDMMC_MMC_SUPPORT_8_BIT_DATA_WIDTH;
