 ...
 *
 * Note: if you want to use DAT3 as card detect pin, please make sure the DAT3
 * is pulled down with 100K resistor on board, it is not suggest to use the
 * internal pull down function, from our test result, internal pull down is too
 * strong to cover all the card. And please pay attention, DAT3 card detection
 * cannot works during the card access, since the DAT3 will be used for data
 * transfer, thus the functionality of card detect interrupt will be disabled as
 * soon as card is detected. So If application would like to re-detect
 * sdcard/sdiocard, please calling SD_PollingCardInsert/SDIO_PollingCardInsert
 * The function will polling the card detect status and could yield CPU while
 * RTOS and non-blocking adapter is using. DAT3 card detect maynot able to cover
 * all the card(wifi/sdcard), as the difference of the card driver strength,
 ...
 */
#ifdef SD_ENABLED
  extern void BOARD_SD_Config( void *card, sd_cd_t cd, uint32_t hostIRQPriority,
                               void *userData );
#endif
