...
#define MSD_THROUGHPUT_TEST_ENABLE ( 1 )
...
typedef struct _usb_host_msd_command_instance
{
  ...
  uint8_t runWaitState; /*!< application wait status, go to next run status when
                           the wait status success */
 ...
 * successfully.
 * @retval kStatus_USB_NotSupported         The configuration doesn't contain msd
 * interface.
 ...
 *
 * This function implements the host msd command action, it is used to create
 * task.
 *
