...
#define MCU_CORE_CLOCK                                                         \
  ( 120000000 ) /* mcu core clock, user need to configure                      \
                     it. */
#warning                                                                       \
  "Please check the value of MCU_CORE_CLOCK to make sure it is the right CPU clock!"
 ...
 */
static void USB_HostMsdUfiCallback( void *param, uint8_t *data,
                                    uint32_t dataLength, usb_status_t status );
 ...
 * @status           transfer result status.
 */
static void USB_HostMsdControlCallback( void *param, uint8_t *data,
                                        uint32_t     dataLength,
                                        usb_status_t status );
 ...
 */
static void
msd_command_test_done( usb_host_msd_command_instance_t *msdCommandInstance );
 ...
 */
static void
USB_HostMsdCommandTest( usb_host_msd_command_instance_t *msdCommandInstance );

/*******************************************************************************
 * Variables
 ******************************************************************************/
extern usb_host_handle          g_HostHandle; /* global host handle */
usb_host_msd_command_instance_t g_MsdCommandInstance = {
  0 }; /* global msd command instance */
/* command on-going state. It should set to 1 when start command, it is set to 0
 * in the callback */
...
static uint8_t s_TestUfiBuffer[512]; /*!< test buffer */

#if MSD_THROUGHPUT_TEST_ENABLE
USB_DMA_NONINIT_DATA_ALIGN( USB_DATA_ALIGN_SIZE )
static uint32_t testThroughputBuffer[THROUGHPUT_BUFFER_SIZE /
                                     4]; /* the buffer for throughput test */
uint32_t        testSizeArray[] = {
  50 * 1024, 50 * 1024 }; /* test time and test size (uint: K) */
#endif                           /* MSD_THROUGHPUT_TEST_ENABLE */
...
static void USB_HostMsdControlCallback( void *param, uint8_t *data,
                                        ...
                                        usb_status_t status )
{
  usb_host_msd_command_instance_t *msdCommandInstance =
    (usb_host_msd_command_instance_t *)param;

  if ( msdCommandInstance->runWaitState ==
       kUSB_HostMsdRunWaitSetInterface ) /* set interface finish */
  {
...
#if ( ( defined USB_HOST_CONFIG_IP3516HS ) &&                                  \
      ( USB_HOST_CONFIG_IP3516HS > 0U ) )
...
USB_HostMsdCommandTest( usb_host_msd_command_instance_t *msdCommandInstance )
{
  ...
  ufiIng = 1;
  status = USB_HostMsdGetMaxLun( msdCommandInstance->classHandle,
                                 msdCommandInstance->testUfiBuffer,
  ...
  ufiIng = 1;
  status =
    USB_HostMsdTestUnitReady( msdCommandInstance->classHandle, 0,
                              USB_HostMsdUfiCallback, msdCommandInstance );
  if ( status != kStatus_USB_Success ) {
    usb_echo( "error\r\n" );
    msd_command_test_done( msdCommandInstance );
  ...
  if ( ( ufiStatus == kStatus_USB_Success ) ||
       ( ufiStatus ==
         kStatus_USB_MSDStatusFail ) ) /* print the command result */
  {
  ...
  ufiIng = 1;
  status = USB_HostMsdRequestSense(
    msdCommandInstance->classHandle, 0, msdCommandInstance->testUfiBuffer,
    sizeof( usb_host_ufi_sense_data_t ), USB_HostMsdUfiCallback,
    msdCommandInstance );
  if ( status != kStatus_USB_Success ) {
    usb_echo( "error\r\n" );
    msd_command_test_done( msdCommandInstance );
  ...
  usb_echo( "inquiry..." );
  ufiIng = 1;
  status = USB_HostMsdInquiry( msdCommandInstance->classHandle, 0,
                               msdCommandInstance->testUfiBuffer,
                               sizeof( usb_host_ufi_inquiry_data_t ),
                               USB_HostMsdUfiCallback, msdCommandInstance );
  if ( status != kStatus_USB_Success ) {
    usb_echo( "error\r\n" );
    msd_command_test_done( msdCommandInstance );
  ...
  ufiIng = 1;
  status = USB_HostMsdReadCapacity(
    msdCommandInstance->classHandle, 0, msdCommandInstance->testUfiBuffer,
    sizeof( usb_host_ufi_read_capacity_t ), USB_HostMsdUfiCallback,
    msdCommandInstance );
  if ( status != kStatus_USB_Success ) {
    usb_echo( "error\r\n" );
    msd_command_test_done( msdCommandInstance );
  ...
  if ( ufiStatus == kStatus_USB_Success ) /* print the command result */
  {
    ...
    address = (uint32_t)( (usb_host_ufi_read_capacity_t *)( address ) )
                ->blockLengthInBytes;
    ...
    usb_echo( "success, last logical block:%d block length:%d\r\n",
              USB_LONG_FROM_BIG_ENDIAN_ADDRESS( ( (uint8_t *)address ) ),
              blockSize );
  }
  else {
    usb_echo( "fail\r\n" );
  ...
  usb_echo( "read(10)..." );
  ufiIng = 1;
  status = USB_HostMsdRead10( msdCommandInstance->classHandle, 0, 0,
                              msdCommandInstance->testUfiBuffer, blockSize, 1,
                              USB_HostMsdUfiCallback, msdCommandInstance );
  if ( status != kStatus_USB_Success ) {
    usb_echo( "error\r\n" );
    msd_command_test_done( msdCommandInstance );
  ...
  usb_echo( "write(10)..." );
  ufiIng = 1;
  status = USB_HostMsdWrite10( msdCommandInstance->classHandle, 0, 0,
                               msdCommandInstance->testUfiBuffer, blockSize, 1,
                               USB_HostMsdUfiCallback, msdCommandInstance );
  if ( status != kStatus_USB_Success ) {
    usb_echo( "error\r\n" );
  ...
  for ( testIndex = 0; testIndex < ( sizeof( testSizeArray ) / 4 );
        ++testIndex ) {
    totalTime    = 0;
    blockAddress = 0;
    ...
    while ( testSize ) {
      ...
      status = USB_HostMsdWrite10(
        msdCommandInstance->classHandle, 0, blockAddress,
        (uint8_t *)&testThroughputBuffer[0], THROUGHPUT_BUFFER_SIZE,
        ( THROUGHPUT_BUFFER_SIZE / blockSize ), USB_HostMsdUfiCallback,
        msdCommandInstance );
      if ( status != kStatus_USB_Success ) {
        usb_echo( "    error\r\n" );
        msd_command_test_done( msdCommandInstance );
    ...
    usb_echo( "    write %dKB data the speed is %d KB/s\r\n", testSize,
              (uint32_t)( (uint64_t)testSize * (uint64_t)MCU_CORE_CLOCK /
                          (uint64_t)totalTime ) );
...
void USB_HostMsdTask( void *arg )
{
  usb_status_t                     status;
  usb_host_msd_command_instance_t *msdCommandInstance =
    (usb_host_msd_command_instance_t *)arg;

  if ( msdCommandInstance->deviceState !=
       msdCommandInstance->prevDeviceState ) {
    ...
    case kStatus_DEV_Attached: /* device is attached and numeration is done */
      status = USB_HostMsdInit(
        msdCommandInstance->deviceHandle,
  ...
  case kUSB_HostMsdRunSetInterface: /* set msd interface */
    ...
    status                           = USB_HostMsdSetInterface(
      msdCommandInstance->classHandle, msdCommandInstance->interfaceHandle, 0,
...
usb_status_t
USB_HostMsdEvent( usb_device_handle             deviceHandle,
                  ...
                  uint32_t                      eventCode )
{
  ...
  case kUSB_HostEventAttach:
    ...
    for ( interfaceIndex = 0; interfaceIndex < configuration->interfaceCount;
      ...
      if ( ( id != USB_HOST_MSD_SUBCLASS_CODE_UFI ) &&
           ( id != USB_HOST_MSD_SUBCLASS_CODE_SCSI ) ) {
        continue;
      }
  ...
  case kUSB_HostEventEnumerationDone:
    if ( g_MsdCommandInstance.configHandle == configurationHandle ) {
      if ( ( g_MsdCommandInstance.deviceHandle != NULL ) &&
           ( g_MsdCommandInstance.interfaceHandle != NULL ) ) {
        /* the device enumeration is done */
        if ( g_MsdCommandInstance.deviceState == kStatus_DEV_Idle ) {
          g_MsdCommandInstance.deviceState = kStatus_DEV_Attached;

          USB_HostHelperGetPeripheralInformation(
            deviceHandle, kUSB_HostGetDevicePID, &infoValue );
          usb_echo( "mass storage device attached:pid=0x%x", infoValue );
          USB_HostHelperGetPeripheralInformation(
            deviceHandle, kUSB_HostGetDeviceVID, &infoValue );
          usb_echo( "vid=0x%x ", infoValue );
          USB_HostHelperGetPeripheralInformation(
            deviceHandle, kUSB_HostGetDeviceAddress, &infoValue );
          usb_echo( "address=%d\r\n", infoValue );
        }
        else {
