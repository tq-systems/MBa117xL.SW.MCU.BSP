...
#include "fsl_lpi2c.h"
#include "PMIC.h"
#if defined(VERSION_INFORMATION) && VERSION_INFORMATION
#include "version.h"
#endif
...
void BOARD_InitDebugConsole(void)
{
  ...
  PRINTF("This is a demo application for the device " BOARD_NAME ".\r\n");
  PRINTF("Welcome to the demonstration. \r\n\r\n");
}
...
status_t BOARD_LPI2C_SendSCCB(LPI2C_Type *base, uint8_t deviceAddress,
                              uint32_t subAddress, uint8_t subAddressSize,
                              uint8_t *txBuff, uint8_t txBuffSize)
{
  lpi2c_master_transfer_t xfer;
...
status_t BOARD_LPI2C_ReceiveSCCB(LPI2C_Type *base, uint8_t deviceAddress,
                                 uint32_t subAddress, uint8_t subAddressSize,
                                 uint8_t *rxBuff, uint8_t rxBuffSize)
{
  status_t                status;
...
status_t BOARD_Accel_I2C_Receive(uint8_t deviceAddress, uint32_t subAddress,
                                 ...
                                 uint8_t rxBuffSize)
{
  return BOARD_LPI2C_Receive(BOARD_ACCEL_I2C_BASEADDR, deviceAddress,
                             subAddress, subaddressSize, rxBuff, rxBuffSize);
}
...
status_t BOARD_Codec_I2C_Send(uint8_t deviceAddress, uint32_t subAddress,
                              uint8_t subAddressSize, const uint8_t *txBuff,
                              uint8_t txBuffSize)
{
  return BOARD_LPI2C_Send(BOARD_CODEC_I2C_BASEADDR, deviceAddress, subAddress,
                          subAddressSize, (uint8_t *) txBuff, txBuffSize);
}

status_t BOARD_Codec_I2C_Receive(uint8_t deviceAddress, uint32_t subAddress,
                                 uint8_t subAddressSize, uint8_t *rxBuff,
                                 uint8_t rxBuffSize)
{
  return BOARD_LPI2C_Receive(BOARD_CODEC_I2C_BASEADDR, deviceAddress,
                             subAddress, subAddressSize, rxBuff, rxBuffSize);
}
...
void BOARD_Camera_I2C_Init(void)
{
  ...
  BOARD_LPI2C_Init(BOARD_CAMERA_I2C_BASEADDR,
                   CLOCK_GetRootClockFreq(BOARD_CAMERA_I2C_CLOCK_ROOT));
}
...
status_t BOARD_Camera_I2C_Send(uint8_t deviceAddress, uint32_t subAddress,
                               ...
                               uint8_t txBuffSize)
{
  return BOARD_LPI2C_Send(BOARD_CAMERA_I2C_BASEADDR, deviceAddress, subAddress,
                          subAddressSize, (uint8_t *) txBuff, txBuffSize);
}
...
status_t BOARD_Camera_I2C_Receive(uint8_t deviceAddress, uint32_t subAddress,
                                  ...
                                  uint8_t rxBuffSize)
{
  return BOARD_LPI2C_Receive(BOARD_CAMERA_I2C_BASEADDR, deviceAddress,
                             subAddress, subAddressSize, rxBuff, rxBuffSize);
}
...
{
  return BOARD_LPI2C_SendSCCB(BOARD_CAMERA_I2C_BASEADDR, deviceAddress,
                              subAddress, subAddressSize, (uint8_t *) txBuff,
                              txBuffSize);
}
...
status_t BOARD_Camera_I2C_ReceiveSCCB(uint8_t  deviceAddress,
                                      ...
                                      uint8_t rxBuffSize)
{
  return BOARD_LPI2C_ReceiveSCCB(BOARD_CAMERA_I2C_BASEADDR, deviceAddress,
                                 subAddress, subAddressSize, rxBuff,
                                 rxBuffSize);
}
...
status_t BOARD_MIPIPanelTouch_I2C_Send(uint8_t        deviceAddress,
                                       uint32_t       subAddress,
                                       uint8_t        subAddressSize,
                                       const uint8_t *txBuff,
                                       uint8_t        txBuffSize)
{
  return BOARD_LPI2C_Send(BOARD_MIPI_PANEL_TOUCH_I2C_BASEADDR, deviceAddress,
                          subAddress, subAddressSize, (uint8_t *) txBuff,
                          txBuffSize);
}

status_t BOARD_MIPIPanelTouch_I2C_Receive(uint8_t  deviceAddress,
                                          uint32_t subAddress,
                                          uint8_t  subAddressSize,
                                          uint8_t *rxBuff, uint8_t rxBuffSize)
{
  return BOARD_LPI2C_Receive(BOARD_MIPI_PANEL_TOUCH_I2C_BASEADDR, deviceAddress,
                             subAddress, subAddressSize, rxBuff, rxBuffSize);
}
#endif /* SDK_I2C_BASED_COMPONENT_USED */
...
void BOARD_ConfigMPU(void)
{
  ...
  /* RW_m_ncache_unused is a auxiliary region which is used to get the whole
   * size of noncache section */
  extern uint32_t Image$$RW_m_ncache_unused$$Base[];
  extern uint32_t Image$$RW_m_ncache_unused$$ZI$$Limit[];
  uint32_t        nonCacheStart = (uint32_t) Image$$RW_m_ncache$$Base;
  uint32_t        size =
    ((uint32_t) Image$$RW_m_ncache_unused$$Base == nonCacheStart)
             ? 0
  ...
  /* MPU configure:
   * Use ARM_MPU_RASR(DisableExec, AccessPermission, TypeExtField, IsShareable,
   * IsCacheable, IsBufferable, SubRegionDisable, Size) API in mpu_armv7.h.
   * param DisableExec       Instruction access (XN) disable bit,0=instruction
   * fetches enabled, 1=instruction fetches disabled. param AccessPermission
   * Data access permissions, allows you to configure read/write access for User
   * and Privileged mode. Use MACROS defined in mpu_armv7.h:
   * ARM_MPU_AP_NONE/ARM_MPU_AP_PRIV/ARM_MPU_AP_URO/ARM_MPU_AP_FULL/ARM_MPU_AP_PRO/ARM_MPU_AP_RO
   * Combine TypeExtField/IsShareable/IsCacheable/IsBufferable to configure MPU
   * memory access attributes. TypeExtField  IsShareable  IsCacheable
   * IsBufferable   Memory Attribtue    Shareability        Cache 0 x 0 0
   * Strongly Ordered    shareable 0             x           0           1
   * Device             shareable 0             0           1           0 Normal
   * not shareable   Outer and inner write through no write allocate 0 0 1 1
   * Normal             not shareable   Outer and inner write back no write
   * allocate 0             1           1           0              Normal
   * shareable       Outer and inner write through no write allocate 0 1 1 1
   * Normal             shareable       Outer and inner write back no write
   * allocate 1             0           0           0              Normal not
   * shareable   outer and inner noncache 1             1           0 0 Normal
   * shareable       outer and inner noncache 1             0           1 1
   * Normal             not shareable   outer and inner write back write/read
   * acllocate 1             1           1           1              Normal
   * shareable       outer and inner write back write/read acllocate 2 x 0 0
   * Device              not shareable Above are normal use settings, if your
   * want to see more details or want to config different inner/outter cache
   * policy.
   *  please refer to Table 4-55 /4-56 in arm cortex-M7 generic user guide
   * <dui0646b_cortex_m7_dgug.pdf> param SubRegionDisable  Sub-region disable
   * field. 0=sub-region is enabled, 1=sub-region is disabled. param Size Region
   * size of the region to be configured. use ARM_MPU_REGION_SIZE_xxx MACRO in
   * mpu_armv7.h.
   */
  ...
  MPU->RASR =
    ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 2, 0, 0, 0, 0, ARM_MPU_REGION_SIZE_512MB);
  ...
  /* Region 4 setting: Memory with Normal type, not shareable, outer/inner write
   * back */
  MPU->RBAR = ARM_MPU_RBAR(4, 0x00000000U);
  MPU->RASR =
    ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 0, 0, 1, 1, 0, ARM_MPU_REGION_SIZE_256KB);

  /* Region 5 setting: Memory with Normal type, not shareable, outer/inner write
   * back */
  MPU->RBAR = ARM_MPU_RBAR(5, 0x20000000U);
  MPU->RASR =
    ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 0, 0, 1, 1, 0, ARM_MPU_REGION_SIZE_256KB);

  /* Region 6 setting: Memory with Normal type, not shareable, outer/inner write
   * back */
  MPU->RBAR = ARM_MPU_RBAR(6, 0x20200000U);
  MPU->RASR =
    ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 0, 0, 1, 1, 0, ARM_MPU_REGION_SIZE_1MB);

  /* Region 7 setting: Memory with Normal type, not shareable, outer/inner write
   * back */
  MPU->RBAR = ARM_MPU_RBAR(7, 0x20300000U);
  MPU->RASR =
    ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 0, 0, 1, 1, 0, ARM_MPU_REGION_SIZE_512KB);

#if defined(XIP_EXTERNAL_FLASH) && (XIP_EXTERNAL_FLASH == 1)
  /* Region 8 setting: Memory with Normal type, not shareable, outer/inner write
   * back. */
...
#ifdef USE_SDRAM
  /* Region 9 setting: Memory with Normal type, not shareable, outer/inner write
   * back */
  MPU->RBAR = ARM_MPU_RBAR(9, 0x80000000U);
  MPU->RASR =
    ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 0, 0, 1, 1, 0, ARM_MPU_REGION_SIZE_64MB);
#endif
  ...
  if (i != 0)
  {
    /* The MPU region size should be 2^N, 5<=N<=32, region base should be
     * multiples of size. */
  ...
  MPU->RASR =
    ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 2, 0, 0, 0, 0, ARM_MPU_REGION_SIZE_16MB);

  /* Region 12 setting: Memory with Device type, not shareable, non-cacheable */
  MPU->RBAR = ARM_MPU_RBAR(12, 0x41000000);
  MPU->RASR =
    ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 2, 0, 0, 0, 0, ARM_MPU_REGION_SIZE_2MB);
...
void BOARD_ConfigMPU(void)
{
  ...
  /* RPMSG_SH_MEM_unused is a auxiliary region which is used to get the whole
   * size of RPMSG_SH_MEM section */
  ...
  uint32_t        rpmsgShmemSize =
    (uint32_t) Image$$RPMSG_SH_MEM_unused$$ZI$$Limit - rpmsgShmemStart;
#elif defined(__MCUXPRESSO)
  extern uint32_t __base_rpmsg_sh_mem;
  extern uint32_t __top_rpmsg_sh_mem;
  ...
  if (LMEM_PCCCR_ENCACHE_MASK == (LMEM_PCCCR_ENCACHE_MASK & LMEM->PCCCR))
  {
    ...
    LMEM->PCCCR |=
      LMEM_PCCCR_PUSHW0_MASK | LMEM_PCCCR_PUSHW1_MASK | LMEM_PCCCR_GO_MASK;
  ...
  if (LMEM_PSCCR_ENCACHE_MASK == (LMEM_PSCCR_ENCACHE_MASK & LMEM->PSCCR))
  {
    ...
    LMEM->PSCCR |=
      LMEM_PSCCR_PUSHW0_MASK | LMEM_PSCCR_PUSHW1_MASK | LMEM_PSCCR_GO_MASK;
    /* Wait until the cache command completes. */
  ...
  LMEM->PSCCR |=
    LMEM_PSCCR_INVW0_MASK | LMEM_PSCCR_INVW1_MASK | LMEM_PSCCR_GO_MASK;
  /* Wait until the cache command completes */
  while ((LMEM->PSCCR & LMEM_PSCCR_GO_MASK) != 0U)
  {
  }
  ...
  LMEM->PCCCR |=
    LMEM_PCCCR_INVW0_MASK | LMEM_PCCCR_INVW1_MASK | LMEM_PCCCR_GO_MASK;
  /* Wait until the cache command completes. */
  while ((LMEM->PCCCR & LMEM_PCCCR_GO_MASK) != 0U)
  {
  }
...
/* PMIC hast to be set at proper voltage for enabling BOARD work properly at
 * overdrive-mode */
