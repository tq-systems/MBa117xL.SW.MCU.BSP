...
#define BOARD_NAME "TQMa1176L-MBa117xL"
#define MANUFACTURER_NAME "TQ-Systems GmbH"
...
#define BOARD_USER_LED_GPIO BOARD_INITPINS_USER_LED_1_PERIPHERAL
#endif
#ifndef BOARD_USER_LED_GPIO_PIN
#define BOARD_USER_LED_GPIO_PIN BOARD_INITPINS_USER_LED_1_CHANNEL
#endif

#define USER_LED_INIT(output)                                                  \
  GPIO_PinWrite(BOARD_USER_LED_GPIO, BOARD_USER_LED_GPIO_PIN, output);         \
  BOARD_USER_LED_GPIO->GDIR |=                                                 \
    (1U << BOARD_USER_LED_GPIO_PIN) /*!< Enable target USER_LED */
#define USER_LED_OFF()                                                         \
  GPIO_PortClear(                                                              \
    BOARD_USER_LED_GPIO,                                                       \
    1U << BOARD_USER_LED_GPIO_PIN) /*!< Turn off target USER_LED */
#define USER_LED_ON()                                                          \
  GPIO_PortSet(BOARD_USER_LED_GPIO,                                            \
               1U << BOARD_USER_LED_GPIO_PIN) /*!<Turn on target USER_LED*/
#define USER_LED_TOGGLE()                                                      \
  GPIO_PinWrite(                                                               \
    BOARD_USER_LED_GPIO, BOARD_USER_LED_GPIO_PIN,                              \
    0x1                                                                        \
      ^ GPIO_PinRead(BOARD_USER_LED_GPIO,                                      \
                     BOARD_USER_LED_GPIO_PIN)) /*!< Toggle target USER_LED */
...
#define BOARD_SMARTCARD_MODULE                                                 \
  (EMVSIM1) /*!< SMARTCARD communicational module instance */
#define BOARD_SMARTCARD_MODULE_IRQ                                             \
  (EMVSIM1_IRQn) /*!< SMARTCARD communicational module IRQ handler */
#define BOARD_SMARTCARD_CLOCK_MODULE_CLK_FREQ                                  \
  (CLOCK_GetRootClockFreq(kCLOCK_Root_Emv1))
...
#define BOARD_CAMERA_I2C_CLOCK_DIVIDER                                         \
  (12U) /* Divider = 12, LPI2C clock frequency 2M. */
...
#define BOARD_MIPI_PANEL_TOUCH_I2C_CLOCK_DIVIDER                               \
  (12U) /* Divider = 12, LPI2C clock frequency 2M. */
#define BOARD_MIPI_PANEL_TOUCH_I2C_CLOCK_FREQ                                  \
