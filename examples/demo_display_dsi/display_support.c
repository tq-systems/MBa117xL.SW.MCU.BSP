...
#define DEMO_LCDIF_POL_FLAGS                                                   \
  (kELCDIF_DataEnableActiveHigh | kELCDIF_VsyncActiveLow                       \
   | kELCDIF_HsyncActiveLow | kELCDIF_DriveDataOnFallingClkEdge)
...
#define MIPI_DSI_SEL_PIN 0x20
#define MIPI_RST_LCD_PIN 0x04
#define MIPI_PW_EN_PIN 0x08
#define MIPI_BL_PIN 0x02
...
dc_fb_elcdif_handle_t s_dcFbElcdifHandle = {
  0}; /* The handle must be initialized to 0. */
...
static void BOARD_PullPanelResetPin(bool pullUp)
{
  if (pullUp)
  {
    portExpander.OutputMask.PIN10_PIN17_Mask |= MIPI_RST_LCD_PIN;
  }
  else
  {
    portExpander.OutputMask.PIN10_PIN17_Mask &= ~MIPI_RST_LCD_PIN;
...
static void BOARD_PullPanelPowerPin(bool pullUp)
{
  if (pullUp)
  {
    portExpander.OutputMask.PIN10_PIN17_Mask |= MIPI_PW_EN_PIN;
  }
  else
  {
    portExpander.OutputMask.PIN10_PIN17_Mask &= ~MIPI_PW_EN_PIN;
...
#error "Only LPI2C supported."
  ...
  {
    portExpander.OutputMask.PIN10_PIN17_Mask |= MIPI_BL_PIN;
    PCA9555BS_setOutput(&portExpander);
  }
