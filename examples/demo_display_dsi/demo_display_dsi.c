...
#if (!defined(FLASH_TARGET) && !defined(USE_SDRAM))
#error "For this application, please use the linker script for SD-Ram-Flash."
#endif
...
AT_NONCACHEABLE_SECTION_ALIGN(
  static uint8_t s_frameBuffer[DEMO_BUFFER_COUNT][DEMO_BUFFER_HEIGHT]
                              [DEMO_BUFFER_WIDTH][DEMO_BUFFER_BYTE_PER_PIXEL],
  FRAME_BUFFER_ALIGN);
...
static const uint32_t s_frameBufferAddress[DEMO_BUFFER_COUNT] = {
  DEMO_BUFFER0_ADDR,
#if DEMO_BUFFER_COUNT > 1
  DEMO_BUFFER1_ADDR,
#endif
...
static void BOARD_ResetDisplayMix(void)
{
  ...
  while (kSRC_SliceResetInProcess
         == SRC_GetSliceResetState(SRC, kSRC_DisplaySlice))
  {
  }
}
...
int main(void)
{
  ...
  if (xTaskCreate(fbdev_task, "fbdev_task", configMINIMAL_STACK_SIZE + 200,
...
static void DEMO_FillFrameBuffer(void *buffer, uint16_t height, uint16_t width,
                                 uint16_t strideBytes)
{
  ...
  for (i = upperLeftY; i <= lowerRightY; i++)
  {
    for (j = upperLeftX; j <= lowerRightX; j++)
    {
      ...
      *(((uint8_t *) buffer) + strideBytes * i + 3 * j + 1) =
        (fgColor >> 8) & 0xFF;
      *(((uint8_t *) buffer) + strideBytes * i + 3 * j + 2) =
        (fgColor >> 16) & 0xFF;
#elif (4 == DEMO_BUFFER_BYTE_PER_PIXEL)
...
static void fbdev_task(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    DEMO_FillFrameBuffer(buffer, g_fbInfo.bufInfo.height,
                         g_fbInfo.bufInfo.width, g_fbInfo.bufInfo.strideBytes);
     ...
     *
     * NOTE: The project spends time with filling frame buffer, so the
     * calculated frame update rate might be smaller the actual LCD frame
     * refresh rate, especially when frame buffer is large.
     */
    n++;
    if (n > 60)
    {
